# MobileDogs_API #
Выполнили: Конова и Галиулина

## Описание проекта ##
Создать приложение, в котором пользователи могут выбрать бездомных собак, за которыми они будут следить. При добавлении собаки пользователю будут отправляться данные о её передвижении и состоянии. Также пользователи смогут давать задания, касающиеся помощи собакам, другим пользователям, если по какой-то причине он сам не имеет возможности помочь

## Роль устройств(ошейников): ##
1. Отслеживания местоположения собак
2. Отслеживания их состояния здоровья по движению(если долго не движется, то что-то не так)

## Сценарии: ##
1. Авторизация пользователей: пользователь вводит свой логин, пароль и токен -> если в базе данных уже есть такой номер и их пароль и токен, то пользователь входит в аккаунт
2. Регистрация пользователей: пользователь вводит свой логин, пароль -> в базе создаётся новая запись с его уникальным id, логином, паролем и токеном. Так же пользователю выводится его токен, с помощью которого он сможет выполнять определённые действия
3. Регистрация ошейников: при активации ошейника на сервер отправляются код ошейника и кличка собаки -> создаётся запись в базе данных с кодом ошейника, кличкой собаки и токеном ошейника
4. Привязка ошейников﻿ и пользователей: пользователь смотрит фото собак поблизости и выбирает какую добавить -> пользователь отправляет на сервер запрос с id ошейника выбранной собаки и из его профиля берётся его id -> в базе данных для связи пользователей и собак создаётся новая запись с полями id пользователя, id ошейника и токен пользователя
5. Оповощение пользователей: берется id ошейника и проверяется у каких пользователей есть этот id в таблице -> пользователям, привязанным к собаке отправляется данные с ошейника этой собаки, если, допустим, она давно не двигалась 
6. Модерирование: если у пользователя низкий уровень заряда на телефоне, или состояние ошейника плохое, или подвижность собаки нулевая уже долгое время, то ему приходит уведомление
7. Создание и взятие заданий: пользователь может создать задания, введя свой токен и id собаки, для которой он хочет создать задание -> другие пользователи могут вывести список заданий и посмотреть какие взяты, а какие нет и взять себе задание для выполнения -> после взятия задания пользователь сможет просматривать его у себя в списке заданий 
8. Если собака не двигается: собака долго не двигаетсся -> ошейник подаёт сигнал пользователям, к которым добавлена эта собака или админу, если устройство ни на кого не зарегистрировано -> пользователь идёт к собаке и проверяет жива ли она, или даёт это заание другим пользователям

## Описание таблиц ##
### 1. users (usersT) - хранит данные о пользователях

Столбцы:
- id
- login
- password
- accessToken

### 2. tasks (tasksT) - хранит дпнные о заданиях

Столбцы:
- id
- user_token
- colar_id
- text - текст задания
- taken - статус взято/не взято 

### 3. responses (responsesT) - таблица с откликами на задания

Столбцы:
- id 
- do_user_id
- task_id
- answer
- readiness - готовноть задания

### 4. subscriptions (subsT) - таблица с подписками на собак

Столбцы:
- id
- user_login
- collar_id
- accessToken


## Необходимые запросы, чтобы реализовать сценарии выше: ##
### 1. Регистрация пользователя:
Что вводит пользователь:
```
  {
  login: str - логин пользователя
  password: str - пароль пользователя
  }
```
Если логин пользователя не занят, то пользователю будет выдан токен:
```
  {
  success: bool
  accessToken: str
  }
```
Если такой пользователь уже существует, то будет вызвано исклюение _**BusyLogin**_

### 2. Авторизация:
Что вводит пользователь:
```
  {
  login: str - логин пользователя
  password: str - пароль пользователя
  accessToken: str - токен пользователя
  }
```
Если всё введено правильно, то пользоватулю выдаётся сообщение о том, что он успешно вошёл в аккаунт:
```
  {
  success: bool 
  message: str
  }
```
Если в базе данных нет польователя с таким логином, то будет вызвано исключение _**WrongLogin**_

Если введённый пароль не совпадает с паролем выбранного пользователя, то будет вызвано исключение _**WrongPassword**_

Если введённый токен не совпадает с токеном выбранного пользователя, то будет вызвано исключение _**WrongToken**_

### 3. Регистрация собаки:
Что вводит пользователь:
```
  {
  name: str - кличка собаки
  collar_id: str - id регистрируемого ошейника
  }
```
Если ошейник с таким id не занят, то будет выдан токен ошейника:
```
  {
  success: bool
  accessToken: str
  }
```
Если ошейник с таким id уже занят, то будет вызвано исклюение _**BusyCollar**_

### 4. Создание подписки на собаку:
```
   {
   "user_login": "string" - логин пользователя
   "collar_id": "string" - id ошейника собаки, на которую пользователь подписывается
   "accessToken": "string" - токен пользователя
   }
```
Если пользователь еще не подписан на эту собаку, то пользователю будет возвращен его токен:
```
   {
   "success": true,
   "accessToken": "string" 
   }
```
Если ошейник с таким id не присутствует в базе данных, то будет вызвано исклюение _**DogDoesntExist**_

Если пользователь с таким логином не присутствует в базе данных, то будет вызвано исклюение _**UserDoesntExist**_

Если введенный токен не совпадает с токеном пользователя, то будет вызвано исклюение _**WrongToken**_

Если пользователь уже подписан на собаку, то будет вызвано исклюение _**ExistSubscr**_

### 5. Отписка от собаки:
Что вводит пользователь:
```
   {
   "user_login": "string" - логин пользователя
   "collar_id": "string" - id ошейника собаки, от которой пользователь отписывается
   "accessToken": "string" - токен пользователя
   }
```
Если пользователь подписан на эту собаку, то ему будет возвращено сообщение:
```
   {
   "success": true,
   "accessToken": "string" 
   }
```
Если ошейник с таким id не присутствует в базе данных, то будет вызвано исклюение _**DogDoesntExist**_

Если пользователь с таким логином не присутствует в базе данных, то будет вызвано исклюение _**UserDoesntExist**_

Если введенный токен не совпадает с токеном пользователя, то будет вызвано исклюение _**WrongToken**_

Если пользователь не был подписан на это собаку, то будет вызвано исклюение _**SubscrDoesntExist**_

### 6. Взятие подписок пользователя:
```
   {
   "user_login": "string" - логин пользователя
   "accessToken": "string" - токен пользователя
   }
```
Пользователю будет возвращен список его подписок:
```
   {
   "subs": "string" 
   }
```
Если пользователь с таким логином не присутствует в базе данных, то будет вызвано исклюение _**UserDoesntExist**_

Если введенный токен не совпадает с токеном пользователя, то будет вызвано исклюение _**WrongToken**_

### 7. Взятие подписчиков собаки:
```
   {
   "collar_id": "string" - id ошейника собаки, подписчиков которой нужно посмотреть
   }
```
Будет возвращен список подписчиков собаки:
```
   {
   "subs": "string" 
   }
```
Если ошейник с таким id не присутствует в базе данных, то будет вызвано исклюение _**DogDoesntExist**_

### 8. Взятие координат собаки:
```
   {
   "collar_id": "string" - id ошейника собаки, координаты которой нужно посмотреть
   }
```
Будут возвращены координаты собаки:
```
   {
   "latitude": 0,
   "longitude": 0
   }
```
Если ошейник с таким id не присутствует в базе данных, то будет вызвано исклюение _**DogDoesntExist**_

### 9. Создание заданий на собаку:
```
   {
   accessToken: str
   collar_id: int
   task: str
   }
```
Если собаки с таким id ошейника не существует, то будет вызвано исключение _**NotExistCollar**_
Если пользователя с таким токеном не существует, то будет вызвано исклюение _**WrongToken**_

В ответе будет возвращено:

```
   {
   task_id: int - id созданного задания
   success: bool 
   message: str - сообщение об успешнос создани и задания
   }
```
### 10. Показ заданий на определённую собаку:

```
   {
   task_id: int - id созданного задания
   success: bool 
   message: str - сообщение об успешнос создани и задания
   }
```
Могут быть вызваны исключения NotExistCollar и WrongToken

В ответе будет возвращено:

```
   {
   tasks: object - массив с заданиями на собаку, содержащий id задания, пользователя, который его создал, текст задания и его статус(взято/не взято)
   }
```


### 12. Взятие задания на собаку:


```
   {
   accessToken: str
   task_id: int
   }
```
Могут быть вызваны исключения WrongnTaskId и WrongToken

В ответе будет возвращено:

```
   {
   success: bool
   message: str - сообщение о том, что задание взято
   }
```

Так же в таблице с заданиями статус задания поменяется на True, что будет означать, что оно взято


### 13. Вывод заданий пользователя:

```
   {
   accessToken: str - токен пользователя
   }
```

Может быть вызвано исключение WrongToken

В ответе будет возвращено:

```
   {
   tasks: object - массив со взятыми заданиями пользователя, содержащий id задания, id собаки, текст задания и его статус(взято/не взято)
   }
```


### 14. Становление администратором

```
   {
   code: str - специальный код для админа
   }
```
Если код, введённый пользователем совпадает с кодом, который ввёл пользователь, то в ответе пользователю выдаётся токен админа, с помощью которого он сможет выполнять определённые действия

## Установка и использование: ##

1. Необходимо склонировать репозиторий
2. Перейдите в папку MobileDogs_API
3. Напишите команду в консоли python main.py
4. Перейдите по выданному адресу и делайте запросы
